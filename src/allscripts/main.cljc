(ns allscripts.main
  "Kotodama WASM entrypoint for the Allscripts clean-room actor (L4) — Clojure port.
  ...
  facts (`allscripts.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "allscripts")
(def tier "L4")
...
(defn page-limit
  ...
  src/allscripts/page_limit.kotoba."
  [requested]
  (if (pos? requested)
...
(def entity-specs
  [{:entity "Patient"            :plural "patients"            :id-prefix "allscrip_pat"
   ...
   {:entity "Observation"        :plural "observations"        :id-prefix "allscrip_obs"
    :fields [:resourceType :status :effectiveDateTime :valueString :issued] :required [:resourceType]
    :coerce {}                                        :refs {}}
   {:entity "Encounter"          :plural "encounters"          :id-prefix "allscrip_enc"
    :fields [:resourceType :status :serviceType :period]  :required [:resourceType]
    :coerce {}                                        :refs {}}
   {:entity "MedicationRequest"  :plural "medicationrequests"  :id-prefix "allscrip_med"
    :fields [:resourceType :status :intent :authoredOn]   :required [:resourceType]
    :coerce {}                                        :refs {}}
   {:entity "AllergyIntolerance" :plural "allergyintolerances" :id-prefix "allscrip_all"
    :fields [:resourceType :clinicalStatus :criticality :recordedDate] :required [:resourceType]
    :coerce {}                                        :refs {}}
   {:entity "Condition"          :plural "conditions"          :id-prefix "allscrip_con"
...
(defn emit-facts
  "EAVT facts for one record: {\"allscripts.<Entity>/<field>\" v ...}. The datomic
...
(defn handle-create [store entity data]
  (let [{:keys [fields required coerce id-prefix]} (spec-for entity)
        allowed-fields (conj (set fields) :id)]
    (or (some-> (reject-unknown data allowed-fields) (vector 400))
...
(defn healthz [] [{:status "ok" :actor "allscripts-compat" :tier tier :entities entities} 200])
...
(defn start! [] :allscripts-compat/ready)
